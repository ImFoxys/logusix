...
module.exports = class Logusix {
  ...
  backup({ output, backupName, force }) {
    ...
    try {
      fs.readFile(newPath, (err, data) => {
        ...
        if(data?.toString().length > 0 && !force) throw new LogusixError({ message: 'A file could already exists with this provided backupName' });
      });
      

    } catch(err) {
      throw new LogusixError({ message: err.message });
    }
